...
from django.contrib.auth import (
    BACKEND_SESSION_KEY, HASH_SESSION_KEY, SESSION_KEY, load_backend, user_logged_in,
    user_logged_out,
)
...
from .sessions import CookieMiddleware, SessionMiddleware
...
def get_user(scope):
    ...
    try:
        user_id = _get_user_session_key(session,model)
...
def _get_user_session_key(session,model):
    # This value in the session is always serialized to a string, so we need
    # to convert it back to Python whenever we access it.
    return model._meta.pk.to_python(session[SESSION_KEY])
